...
class RobotCommandProcessor:
    ...
    def process_command(self, text: str) -> Dict[str, Any]:
        ...
        请先判断用户的意图（特别地，如果用户说的话询问机器人是否能执行某个具体的动作，也请判断为对应的指令）
        如果你判断意图为聊天，请正常输出回答，intent设置为chat，action设置为unknown，confidence设置为1.0，description设置为识别到的意图或动作
        如果你判断意图为指令，请输出JSON格式，intent设置为command，action设置为识别到的动作，confidence设置为1.0，description设置为识别到的意图或动作，指令对应的动作类型只包括greet, shake_head, nod, bow,others这五种动作，前面四种就是具体的动作，而others就是除了前面四种动作之外的所有动作。
