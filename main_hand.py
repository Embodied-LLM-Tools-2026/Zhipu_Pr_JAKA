...
os.environ["TORCH_HUB_OFFLINE"] = (
    "1"  # 强制torch hub离线模式,silero-vad模型就不需要联网加载权重
)
import torch
import sys

# 将当前目录和voice目录注册为系统路径
...
class VoiceRobotController:
    ...
    def __init__(
        ...
        self.gripper = GripperController('/dev/ttyUSB0')
        # 确保在创建任何 rclpy Node 之前初始化 rclpy；若已初始化则忽略异常
        try:
            rclpy.init()
        except Exception:
            # 已经初始化或其它初始化错误由上层处理
    ...
    def _cleanup_session_audio(self):
        ...
        print(
            f"📊 清理完成: 成功删除 {cleaned_count} 个文件，失败 {failed_count} 个文件"
        )
        ...
        if os.path.exists(temp_dir):
            ...
            for filename in os.listdir(temp_dir):
                if filename.endswith(".wav") and (
                    "tts_" in filename or "tmp" in filename
                ):
    ...
    def _check_persistent_audio(
        self, text: str, category: str = "common"
    ) -> Optional[str]:
        """检查是否存在持久化的音频文件"""
    ...
    def _save_persistent_audio(
        self, text: str, audio_path: str, category: str = "common"
    ):
        """保存音频文件为持久化文件"""
    ...
    def _init_components(
        ...
        if use_voice_input is None:
            ...
            print(
                f"🔧 输入模式: {'语音输入' if self.recorder else '文本输入'} (自动检测)"
            )
    ...
    def _generate_audio_parallel(self, sentences: list) -> list:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=min(len(sentences), 6)
        ) as executor:
            futures = []
    ...
    def _play_audio_sequence(self, audio_paths: list):
        ...
        for i, audio_path in enumerate(valid_audio_paths):
            try:
                print(
                    f"🔊 播放音频 {i+1}/{len(valid_audio_paths)}: {os.path.basename(audio_path)}"
                )
    ...
    def _get_voice_input(self) -> str:
        ...
        if not self.recorder:
            # 文本输入模式
            if self.robot_state == "sleeping":
                prompt = "请输入唤醒词（家卡同学）或输入'stats'查看计时统计: "
            else:
            ...
            try:
                ...
                try:
                    ...
                    if self.recognizer:
                        ...
                        # 检查是否是统计命令
                        if not self.use_voice_input and (
                            "统计" in text or "计时" in text or "时间" in text
                        ):
                            self._show_timing_stats()
                            return ""
                        return text
                    ...
                    try:
                        if os.path.exists(audio_filename):
                            ...
                            self.temp_audio_files.discard(audio_filename)
                            print(
                                f"🗑️ 已删除临时录音文件: {os.path.basename(audio_filename)}"
                            )
                    except Exception as e:
    ...
    def _handle_sleeping_state(self, text: str) -> bool:
        ...
        # 检测唤醒词
        log_info(f"🎤 休眠状态: 检测唤醒词...")
        wake_detected, remaining_text = self.wake_matcher.detect_wake_word(text)
        if wake_detected:
            log_success(f"🎉 唤醒词检测成功: '{text}'")
            print("🎉 机器人被唤醒！")
            print("🤖 你好！我已准备好接受您的指令。")

            self._play_and_execute_action("你好，我是家卡同学，很高兴见到你！", "greet")

            # 切换到唤醒状态
            self.robot_state = "awake"
            log_success(f"✅ 机器人状态切换: sleeping → awake")
            # 唤醒时不需要执行机器人动作，直接准备接收指令
            print("✅ 机器人已唤醒，准备接收指令")

            return True
        else:
            log_warning(f"😴 非唤醒词，继续休眠: '{text}'")
    ...
    def _play_and_execute_action(self, audio_text: str, action: str) -> bool:
        ...
        def execute_action_thread():
            ...
            print(f"模拟执行动作 in _play_and_execute_action: {action}")
            success = True  # 模拟成功
            if success:
    ...
    def _handle_awake_state(self, text: str) -> bool:
        """处理正常聊天或动作指令或退下指令"""
        if not text.strip():
            log_warning(f"⚠️ 唤醒状态: 未识别到有效文本")
            print("未识别到有效文本")
            return False

        # 检测退下指令
        log_info(f"🎤 唤醒状态: 检测退下指令...")
        if self.wake_matcher and self.wake_matcher.detect_dismiss_command(text):
            log_success(f"👋 退下指令检测成功: '{text}'")
            ...
            # 切换到休眠状态
            self.robot_state = "sleeping"
            log_success(f"✅ 机器人状态切换: awake → sleeping")
            print("😴 机器人已进入休眠状态")
    ...
    def _process_action_command(self, text: str) -> bool:
        ...
        print(
            f"⏱️ LLM处理耗时: {llm_duration:.2f}秒，总文本生成耗时: {text_gen_duration:.2f}秒"
        )
        log_success(f"✅ LLM推理完成: intent={intent}, action={action}, confidence={confidence:.2f}")
        ...
        def tts_ready_callback():
            ...
            print(
                f"⏱️ LLM到TTS音频准备好耗时: {tts_ready_time - tts_prepare_start_time:.2f}秒"
            )
        ...
        if intent == "command":
            ...
            print(f"家卡：{description}")
            print("🎵 开始异步TTS生成和播放（同时预热VAD）...")
            audio_file_path = self._play_cached_audio(
                description, tts_ready_callback=tts_ready_callback
            )
        else:
            print("❓ 录制到环境噪声，无法识别")
        ...
        if intent == "command" and (action == "unknown" or confidence != 1):
            ...
            log_warning(f"⚠️ 指令识别失败: action={action}, confidence={confidence}")
            error_audio_path = self._play_cached_audio(
                "抱歉，我不理解这个指令，请重新说一遍"
            )
            ...
            if error_audio_path and os.path.exists(error_audio_path):
                error_audio_duration = self.audio_player.get_audio_duration(
                    error_audio_path
                )
            ...
            print(
                f"📊 错误响应统计: 响应准备:{error_true_response_time:.2f}s | 音频时长:{error_audio_duration:.2f}s"
            )
            log_error(f"❌ 指令处理失败: 返回错误提示音")
            return False
        ...
        print(
            f"📊 性能统计: LLM:{llm_duration:.2f}s | TTS准备:{tts_prepare_duration:.2f}s | 音频时长:{audio_duration:.2f}s | 响应准备:{true_response_time:.2f}s"
        )
    ...
    def _end_response_cycle_timing(self):
        ...
        print(
            f"📊 计时统计: 平均{avg_time:.3f}秒, 最小{self.timing_stats['min_time']:.3f}秒, 最大{self.timing_stats['max_time']:.3f}秒"
        )
    ...
    def _show_timing_stats(self):
        ...
        avg_time = self.timing_stats["total_time"] / self.timing_stats["total_cycles"]
        recent_avg = (
            sum(self.timing_stats["recent_times"])
            / len(self.timing_stats["recent_times"])
        ...
        if self.timing_stats["recent_times"]:
            print(
                f"最近{len(self.timing_stats['recent_times'])}次耗时: {[f'{t:.3f}s' for t in self.timing_stats['recent_times']]}"
            )
...
def main():
    ...
    try:
        ...
        # controller.navigate.navigate_to_bar()

        # 显示当前配置
        if controller.recorder:
            if (
                hasattr(controller.recorder, "advanced_vad")
                and controller.recorder.advanced_vad
            ):
        ...
        if (
            controller.recorder
            and hasattr(controller.recorder, "advanced_vad")
            and controller.recorder.advanced_vad
        ):
            if controller.recorder.advanced_vad.vad_available:
