...
class RobotCommandProcessor:
    ...
    def process_command(self, text: str) -> Dict[str, Any]:
        ...
        2. 指令 - 控制机器人执行动作（支持的动作类型：打招呼/摆手、摇头、点头、鞠躬、拿饮料、其他）
        ...
        如果你判断意图为指令，请输出JSON格式，intent设置为command，action设置为识别到的动作，obj_name设置为识别到的饮料，num设置为要拿的饮料数量，confidence设置为1.0，description设置为识别到的意图或动作，指令对应的动作类型只包括greet, shake_head, nod, bow,get_drink,others这六种动作，前面五种就是具体的动作，而others就是除了前面五种动作之外的所有动作。另外打招呼这个动作和摆摆手这个动作是等价的，所以打招呼和摆摆手都对应greet动作类型。
        ...
        {{
            ...
            "obj_name": "饮料类型（仅当action为get_drink时有效，使用中文描述，饮料类型只有可能是：{self.drink_list}这{len(self.drink_list)}种，如果识别到的饮料是咖啡但不是美式咖啡（如拿铁、卡布奇诺、玛奇朵、澳瑞白、浓缩、意式等），则归类为其它咖啡）",
